    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                float3 mainGlow = (mainTex.rgb * noiseTex.rgb) * input.color.rgb * _TintColor.rgb;
                
                float3 centerGlow = lerp(mainGlow, mainGlow * saturate(maskTex.rgb * saturate(maskTex.rgb - (input.uv0.b * -1.0 + 1.0))), _Usecenterglow);
                
                float alpha = mainTex.a * input.color.a * _TintColor.a * noiseTex.a;
                
                float depth = 0;
                #ifdef _USEDEPTH_ON
                depth = SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, input.screenPos.xy / input.screenPos.w).r;
                ...
                float3 emission = centerGlow * _Emission;
