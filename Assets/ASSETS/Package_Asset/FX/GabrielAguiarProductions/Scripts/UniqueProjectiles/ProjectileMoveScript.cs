        ...
        if (!bounce)
        {
            if (co.gameObject.tag != "Bullet" && !collided)
            {
                ...
                if (hitPrefab != null)
                {
                    ...
                    // Bảo đảm hit VFX tồn tại tối thiểu 3 giây
                    float life = 3f;
                    var ps = hitVFX.GetComponent<ParticleSystem>();
                    if (ps == null)
                    {
                        var psChild = hitVFX.transform.GetChild(0).GetComponent<ParticleSystem>();
                        if (psChild != null) life = Mathf.Max(life, psChild.main.duration);
                    }
                    else
                    {
                        life = Mathf.Max(life, ps.main.duration);
                    }
                    Destroy(hitVFX, life);
                }

                // Giữ lại projectile root tối thiểu trong thời gian ngắn để tránh cắt vfx quá sớm
