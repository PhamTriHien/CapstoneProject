...
namespace Unity.FantasyKingdom
{
    ...
    public class FogController : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            ...
            // Fix: Find all FullScreenPassRendererFeatures instead of assuming index 0 and 1
            var fullScreenFeatures = new List<FullScreenPassRendererFeature>();
            foreach(var f in features)
            {
                if(f is FullScreenPassRendererFeature fs)
                    fullScreenFeatures.Add(fs);
            }

            if (fullScreenFeatures.Count >= 1)
            {
        ...
        void CleanUpFog()
        {
            ...
            for (int i = 0; i < QualitySettings.names.Length; i++)
            {
                ...
                if (fullScreenFeatures.Count >= 1 && _originalHeightFog != null)
                {
                    HeightPass = fullScreenFeatures[0];
                    HeightPass.passMaterial = _originalHeightFog;
                }
                
                if (fullScreenFeatures.Count >= 2 && _originalCubeFog != null)
                {
                    CubePass = fullScreenFeatures[1];
